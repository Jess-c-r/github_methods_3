...
Rating: Unsure, I did something but didn't understand the question because "Generate data *y* and call it *x*"? That makes no sense?
```{python}
import numpy as np
y = np.arange(0, 6, 0.1)
```
...
```{python}
np.random.seed(7)
y_noisy = np.random.normal(3, 5, len(y))
y += y_noisy
```
