...
author: 'Studygroup 2 - Anja, Astrid, Juli, Magnus and Jessica'
date: "17/11-21"
output: pdf_document
---
...
```{python}
np.amax(repmean)
print(np.unravel_index(np.argmax(repmean), repmean.shape))
```
...
____NEED TEXT HERE____
- A lot of noise
- Seems pretty random
- Y values: Ligger meget omkring 0 som er baseline, klart at der er nogle udsving men ikek hele tiden da det jo kun er en sensor vi kigger på
...
y.shape
```
The "repetition" from "data" has the same length (682). Participants must have done a PAS rating for each repetition?
...
logfit = regressor.fit(X_1_2, y_1_2)
logfit.coef_
```
...
```{python}
logscore = logfit.score(X_1_2, y_1_2)
print(logscore)
```
Yes we're overfitting. Probably because we haven't split the data into test and training sets.

###2.1.vi. 
Now apply the _L1_ penalty instead - how many of the coefficients (`.coef_`) are non-zero after this? 
```{python}
np.random.seed(7)
pregressor = LogisticRegression(penalty = 'l1', solver = 'liblinear')
l1logfit = pregressor.fit(X_1_2, y_1_2)
l1logfit.coef_
np.count_nonzero(l1logfit.coef_)
```
...
```{python}
nonzero = np.where(l1logfit.coef_ != 0)
print(nonzero)
print(nonzero[1])

X_red = X_1_2[:,nonzero[1]]


print(X_red)
print(X_red[1]) 

X_red_cov = (X_red.T @ X_red)
X_red_cov.shape 

plt.figure()
plt.imshow(X_red_cov)
plt.colorbar()
plt.show()
```
This one has more covariance than the first one
...
#scaling it
scaler = StandardScaler()
data_equal_scaled = scaler.fit_transform(data_equal)
data_equal_scaled.shape
```
...
svm_linear_scores = cvs(svm_linear, data_equal_scaled, y_equal, cv=5)
# cross validating the radial support vector
svm_radial_scores = cvs(svm_radial, data_equal_scaled, y_equal, cv=5)
## printing the mean of the cross-validated performances
print(np.mean(svm_linear_scores), 3)
print(np.mean(svm_radial_scores), 3)
```

Both aren't very good, but the classifier with the radial basis is a bit better, with an accuracy score of 0.33
...
for i in range(198):
  #Creating data
  X_time2 = data_equal_scaled[:,i]
  
#Cross-validating 
  score2 = cvs(svm_radial, X_time2, y_equal, cv = 5)
