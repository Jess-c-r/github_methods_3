...
pacman::p_load(tidyverse, readbulk, lme4, dplyr, dfoptim, MuMIn, multcomp)
...
m1 <- glmer(correct ~ 1 + (1 | subject), family = "binomial", df)
logLik(m1)
# add a group-level effect of target.frames
m2 <- glmer(correct ~ target.frames + (1 | subject), family = "binomial", df)
logLik(m2)
# add subject-level slopes for target.frames
m3 <- glmer(correct ~ target.frames + (target.frames | subject), family = "binomial", df)
logLik(m3)
```
