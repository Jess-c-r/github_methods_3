...
```{python}
np.random.seed(7)
randomvector = np.random.normal(0, 5, len(x))
y_noisy = y + randomvector
```
...
For some reason I can't plot anything because "fit isn't defined" - but it obviously is :)))

### ii. now run the code chunk below associated with Exercise 1.2.ii - what does X_quadratic amount to?
Rating: iii - I obviously understand that I'm running the code, but the 'amounting to' thing I have no clue about??
...
regressor = LinearRegression()
regressor.fit(X_quadratic, y_noisy)
y_quadratic_hat = regressor.predict(X-quadratic)
...
Don't remember if low or high values are good..
...
```{python}
#Changing the function so there are 40 iterations instead of 20 (first line of code "n_iter =")
class LinearRegressionGD(object):
